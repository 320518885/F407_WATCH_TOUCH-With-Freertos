...
lv_disp_t * lv_disp_drv_register(lv_disp_drv_t * driver)
{
    ...
    if(driver->draw_ctx == NULL) {
        ...
        if(draw_ctx == NULL) return NULL;  // 如果分配失败，返回NULL
        driver->draw_ctx_init(driver, draw_ctx);
        driver->draw_ctx = draw_ctx;  // 赋值给驱动程序的绘图上下文
    }

    lv_memset_00(disp, sizeof(lv_disp_t));  // 清零显示器结构体

    disp->driver = driver;  // 设置显示器驱动

    disp->inv_en_cnt = 1;  // 初始化无效区域计数

    _lv_ll_init(&disp->sync_areas, sizeof(lv_area_t));  // 初始化同步区域列表

    lv_disp_t * disp_def_tmp = disp_def;
    disp_def                 = disp; /* 临时更改默认屏幕，以在新显示器上创建默认屏幕 */
    
    /* 创建刷新定时器 */
    disp->refr_timer = lv_timer_create(_lv_disp_refr_timer, LV_DISP_DEF_REFR_PERIOD, disp);
    LV_ASSERT_MALLOC(disp->refr_timer);
    if(disp->refr_timer == NULL) {
        lv_mem_free(disp);  // 如果分配失败，释放内存
        return NULL;
    }

    // 检查全刷新的条件
    if(driver->full_refresh && driver->draw_buf->size < (uint32_t)driver->hor_res * driver->ver_res) {
        driver->full_refresh = 0;  // 禁用全刷新
        LV_LOG_WARN("full_refresh requires at least screen sized draw buffer(s)");
    }

    disp->bg_color = lv_color_white();  // 设置背景颜色
#if LV_COLOR_SCREEN_TRANSP
    disp->bg_opa = LV_OPA_TRANSP;  // 设置背景透明度
#else
    disp->bg_opa = LV_OPA_COVER;  // 设置背景不透明
#endif
    ...
    if(lv_theme_default_is_inited() == false) {
        ...
        disp->theme = lv_theme_default_get();  // 获取已存在的主题
    }
#endif

    disp->act_scr   = lv_obj_create(NULL);  // 在显示器上创建默认屏幕
    disp->top_layer = lv_obj_create(NULL);  // 在显示器上创建顶层
    disp->sys_layer = lv_obj_create(NULL);  // 在显示器上创建系统层
    lv_obj_remove_style_all(disp->top_layer);  // 移除顶层的所有样式
    lv_obj_remove_style_all(disp->sys_layer);  // 移除系统层的所有样式
    lv_obj_clear_flag(disp->top_layer, LV_OBJ_FLAG_CLICKABLE);  // 设置顶层为不可点击
    lv_obj_clear_flag(disp->sys_layer, LV_OBJ_FLAG_CLICKABLE);  // 设置系统层为不可点击

    lv_obj_set_scrollbar_mode(disp->top_layer, LV_SCROLLBAR_MODE_OFF);  // 关闭顶层的滚动条
    lv_obj_set_scrollbar_mode(disp->sys_layer, LV_SCROLLBAR_MODE_OFF);   // 关闭系统层的滚动条

    lv_obj_invalidate(disp->act_scr);  // 标记当前屏幕为无效，以便重新绘制

    disp_def = disp_def_tmp;  // 恢复默认显示器
    if(disp_def == NULL) disp_def = disp;  // 如果没有默认显示器，则初始化为当前显示器

    lv_timer_ready(disp->refr_timer);  // 确保屏幕在启动时立即刷新

    return disp;  // 返回新注册的显示器
}
